...
class Boid {
  ...
  avoid(obstacles) {
    ...
    let ahead2 = ahead.copy().mult(0.5); // Point intermédiaire
  
    // Détection de l'obstacle le plus proche
    let obstacleLePlusProche = this.getObstacleLePlusProche(obstacles);
  
    // Si aucun obstacle proche, retourner un vecteur nul
    if (!obstacleLePlusProche) {
      return createVector(0, 0);
    }
  
    // Calcul des distances par rapport au vecteur `ahead` et `ahead2`
    ...
    // Vérification de collision potentielle
    if (distance < obstacleLePlusProche.r + this.r) {
      let desiredVelocity;
      if (distance1 < distance2) {
        desiredVelocity = p5.Vector.sub(ahead, obstacleLePlusProche.pos);
      } else {
        desiredVelocity = p5.Vector.sub(ahead2, obstacleLePlusProche.pos);
      }
